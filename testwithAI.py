...
def register_user(email, password, name):
    """Inserts a new user with 4 fields: _id (Email), name, password, and created_at."""
    user_coll = get_user_collection()
    if user_coll.find_one({"_id": email}):
        return False, "User already exists with this email."
    try:
        user_coll.insert_one({
            "_id": email,           # Field 1: Email
            "name": name,           # Field 2: Name
            "password": password,   # Field 3: Password
            "created_at": datetime.datetime.now() # Field 4: Timestamp
        })
...
except: LLM_MODEL_INSTANCE = None
...
# ---------------- DASHBOARD ----------------
    ...
    if not st.session_state.filter_applied:
        ...
        # (Content filtering logic for 3rd Year 5th Sem as per your structure)
        if st.session_state.submitted_year == "3rd Year" and st.session_state.submitted_sem == "5th Sem":
            if st.session_state.selected_subject_view and st.session_state.selected_resource_type:
                ...
                sub = st.session_state.selected_subject_view
                st.markdown(f"## 🛠️ {sub} - Select Category")
